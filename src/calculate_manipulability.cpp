...
/**
 * Map manipulability value to RGB
 * 
 */
double vmin=0;
double vmax=1;
std::vector<double> manip_val_list;
std_msgs::ColorRGBA mapColor(double v)
{
    ...
    cout << "vmin,max,v,color:"<<vmin<<","<<vmax<<","<<v<<","<<color.r<<","<<color.g<<","<<color.b <<endl;
    return color;
}
...
/**
 * Calculates the manipulability of a robot
 * @param argc Not used
 * @param argv Not used
 * @return 0 on success
 */
int main(int argc, char **argv) {
    ...
    // const auto model = //robot_model_loader.getURDF();
    const auto chain = manipulability_metrics::Chain{ model, root_link, tip_link };
    const auto chain_joints = utils::getParam<std::vector<std::string>>(node_handle, "chain_joints");
    ...
    text_pose.position.x = 0.0;
    text_pose.position.y = 0.0;
    text_pose.position.z = 1.75;
    text_pose.orientation.x = 1.0;
    text_pose.orientation.y = 0.0;
    text_pose.orientation.z = 0.0;
    ...
    for (geometry_msgs::Pose &target_pose : target_poses) { // For all target poses do ...
        if (ros::ok()) { // Check if shutdown is requested, otherwise connection to move it and out stream would be lost and all markers will generated with "unsuccessful" state
            ...
            if (success) {
                ...
                // int size2 = static_cast<int>(my_plan.trajectory_.joint_trajectory.points[0].positions.size());

                cout << "size:" <<size1 <<", "<<size2<<endl; //15, 6
                cout << my_plan.trajectory_.joint_trajectory.points[0]<<endl;//.joint_names() <<endl;
                cout << my_plan.trajectory_.joint_trajectory.joint_names.size() <<endl;
    ...
    // Assign Color
    // ^^^^^^^^^^^^^^^^^
    std::vector<double> tmp = manip_val_list;

    tmp.erase(std::remove(tmp.begin(), tmp.end(), -1.0), tmp.end());
    auto ori_minmax = minmax_element(manip_val_list.begin(), manip_val_list.end());
    auto minmax = minmax_element(tmp.begin(), tmp.end());
    cout << "\npair_min = " << *minmax.first<< ", pair_max = " << *minmax.second << '\n';
    cout << "\nori_min = " << *ori_minmax.first<< ", ori__max = " << *ori_minmax.second << '\n';
    vmin = *minmax.first;    
    vmax = *minmax.second;
    for(int cnt=0; cnt<manip_val_list.size(); cnt++)
    {
...
//         k++;
//         ROS_INFO("Positions for point = %i", i);
//         ROS_INFO("Floatvalue for position = %g", position[k]);
//     }
// }